export default function(ngapp) {
    ...
    ngapp.directive('mainTreeNodes', function ($compile, $timeout) {
        let buildRows = function(nodes) {
            return nodes.map(function(node, index) {
                return `<div ng-if="1" ng-init="node = nodes[${index}]"><main-tree-node></main-tree-node></div>`;
            });
        };

        return {
            restrict: 'E',
            template: '<div class="clusterize-content"></div>',
            link: function(scope, element) {
                // Clusterize elements
                var contentEl = element[0].firstElementChild;
                var scrollEl =  element[0].parentNode;

                // init clusterize
                var clusterize = new Clusterize({
                    rows: buildRows(scope.nodes),
                    scrollElem: scrollEl,
                ...
                scope.$watch('nodes.length', function() {
                    clusterize.update(buildRows(scope.nodes));
                });
            }
        }
    });
